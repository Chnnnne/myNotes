...
`java.io.File` 类是**文件和目录**路径名的抽象表示，主要用于**文件和目录**的创建、查找和删除等操作。
...
> 小贴士：
>
> 1. 一个File对象代表硬盘中实际存在的**一个 文件 或者 目录**。
...
* **相对路径**：相对于项目目录的路径，这是一个便捷的路径，开发中经常使用。     ==工程目录下为根路径==
...
- `public boolean createNewFile()` ：不存在则创建，已存在什么都不做。  返回是否创建
